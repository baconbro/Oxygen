...
class StepperComponent {
  ...
  constructor(_element: HTMLElement, options: IStepperOptions) {
    ...
    // Elements
    this.steps = this.element.querySelectorAll('[data-xgn-stepper-element="nav"]')
    this.btnNext = this.element.querySelector('[data-xgn-stepper-action="next"]')
    this.btnPrev = this.element.querySelector('[data-xgn-stepper-action="previous"]')
    this.btnSubmit = this.element.querySelector('[data-xgn-stepper-action="submit"]')
  ...
  private _goTo = (index: number) => {
    EventHandlerUtil.trigger(this.element, 'xgn.stepper.change')
    ...
    EventHandlerUtil.trigger(this.element, 'xgn.stepper.changed')
  }
  ...
  private initHandlers = () => {
    this.btnNext?.addEventListener('click', (e: Event) => {
      ...
      EventHandlerUtil.trigger(this.element, 'xgn.stepper.next', e)
    })

    this.btnPrev?.addEventListener('click', (e: Event) => {
      e.preventDefault()

      EventHandlerUtil.trigger(this.element, 'xgn.stepper.previous', e)
    })

    DOMEventHandlerUtil.on(this.element, '[data-xgn-stepper-action="step"]', 'click', (e: Event) => {
      e.preventDefault()
  ...
  private getStepContent = (index: number) => {
    const content = this.element.querySelectorAll('[data-xgn-stepper-element="content"]')
    if (!content) {
      return false
    }
  ...
  private refreshUI = () => {
    ...
    const elements = this.element.querySelectorAll(
      '[data-xgn-stepper-element="nav"], [data-xgn-stepper-element="content"], [data-xgn-stepper-element="info"]'
    )
    ...
    for (var i = 0, len = elements.length; i < len; i++) {
      ...
      if (index === this.currentStepIndex) {
        ...
        if (
          ...
          element.getAttribute('data-xgn-stepper-element') === 'content'
        ) {
  ...
  public static bootstrap(attr: string = '[data-xgn-stepper]') {
