...
export class LayoutSetup {
  ...
  private static initLayout(config: ILayout): void {
    ...
    document.body.setAttribute('id', 'xgn_body')
  ...
  private static initToolbar(config: IToolbar): void {
    ...
    if (typeOptions) {
      ...
      if (typeOptions.heightAndTabletMobileMode) {
        bodyStyles += ` --xgn-toolbar-height-tablet-and-mobile: ${typeOptions.heightAndTabletMobileMode};`
      }
  ...
  private static initPageTitle(config: IPageTitle): void {
    ...
    if (config.responsive) {
      ...
      LayoutSetup.attributes.pageTitle.set('data-xgn-swapper', true)
      LayoutSetup.attributes.pageTitle.set('data-xgn-swapper-mode', 'prepend')
      LayoutSetup.attributes.pageTitle.set(
        'data-xgn-swapper-parent',
        `{ default: '#xgn_content_container', '${config.responsiveBreakpoint}': '${config.responsiveTarget}'}`
      )
    }
  }
