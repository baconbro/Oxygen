...
const TaskDependencies = ({ issue, updateIssue, modalClose, scope }) => {
  ...
  const deleteDependencie = (id) => {
    const dependencyToRemove = dependencies.find((dep) => dep.id === id.id);
    const newDependencies = dependencies.filter((dep) => dep.id !== id.id);

    // Update current issue's dependencies
    const mutateItem = editItemMutation({
      orgId: currentUser?.all?.currentOrg,
      field: { dependencies: newDependencies },
      itemId: issue.id,
    ...
    // Find the related issue and remove the dependency from it
    if (dependencyToRemove) {
      const relatedIssue = getIssue(dependencyToRemove.B);
      if (relatedIssue && relatedIssue.dependencies) {
        const relatedIssueDeps = relatedIssue.dependencies.filter(
          (dep) => !(dep.A === dependencyToRemove.B && dep.B === dependencyToRemove.A)
        );

        // Update related issue's dependencies
        editItemMutation({
          orgId: currentUser?.all?.currentOrg,
          field: { dependencies: relatedIssueDeps },
          itemId: relatedIssue.id,
          workspaceId: relatedIssue.projectId || issue.projectId,
        });
      }
    }
  };

  const getReverseDependencyType = (type) => {
    switch (type) {
      case DepType.BLOCKED_BY:
        return DepType.BLOCKS;
      case DepType.BLOCKS:
        return DepType.BLOCKED_BY;
      case DepType.DUPLICATES:
        return DepType.DUPLICATED_BY;
      case DepType.DUPLICATED_BY:
        return DepType.DUPLICATES;
      case DepType.RELATES_TO:
        return DepType.RELATES_TO;
      default:
        return DepType.RELATES_TO;
    }
  };
  ...
  const handleAddDependency = (newDependency) => {
    ...
    // Add corresponding dependency to the related issue
    const relatedIssue = getIssue(newDependency.B);
    if (relatedIssue) {
      // Create corresponding dependency with reversed relationship
      const correspondingDependency = {
        id: Date.now(), // Generate a new unique ID
        A: newDependency.B, // Swap A and B
        B: newDependency.A,
        type: getReverseDependencyType(newDependency.type),
        description: newDependency.description,
      };

      const relatedIssueDeps = relatedIssue.dependencies || [];
      const updatedRelatedDeps = [...relatedIssueDeps, correspondingDependency];

      // Update related issue's dependencies
      editItemMutation({
        orgId: currentUser?.all?.currentOrg,
        field: { dependencies: updatedRelatedDeps },
        itemId: relatedIssue.id,
        workspaceId: relatedIssue.projectId || issue.projectId,
      });
    }
  ...
  return (
    <>
      ...
      {dependencies.length > 0 &&
        Object.values(dependencies).map((values, index) => {
          return (
            <div
              className="m-0 row border-gray-300 border-dashed rounded pb-1 mb-6"
              key={index}
            >
              <div className="timeline ms-n1 col-5">
                <div className="timeline-item align-items-center mb-4">
                  ...
                  <div className="timeline-content m-0">
                    <a
                      href="#"
                      className="fs-6 text-gray-800 fw-bold d-block text-hover-primary"
                    >
                      {' '}
                      <IssueTypeIcon
                        type={getIssue(values.A).type}
                        top={1}
                        className="svg-icon-2 svg-icon-primary"
                      />{' '}
                      {getIssue(values.A).title}
                    </a>
                    <span className="fw-semibold text-gray-400">
                      {getIssue(values.A).id}
                    </span>
                  </div>
                </div>
                ...
                <div className="timeline-item align-items-center">
                  ...
                  <div className=" m-0 ">
                    <Select
                      ...
                      onChange={(type) =>
                        updateDependencies({ type }, { id: values.id })
                      }
                      renderValue={({ value: type }) => (
                        <span className="fs-8 fw-bolder text-info text-uppercase">
                          {type}
                        </span>
                      )}
                    ...
                    >
                      {' '}
                      <IssueTypeIcon
                        type={getIssue(values.B).type}
                        top={1}
                        className="svg-icon-2 svg-icon-primary"
                      />
                      {getIssue(values.B).title}
                    </a>
                    <span className="fw-semibold text-gray-400">
                      {getIssue(values.B).id}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="ms-n1 col-1 d-flex flex-column-fluid flex-center">
                <i
                  className="bi bi-trash fs-2 text-danger"
                  onClick={() => handleDeleteClick({ id: values.id })}
                ></i>
              </div>
            </div>
          );
